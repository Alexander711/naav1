...
class Controller_Services_Main extends Controller_Frontend {	
    ...
    {
        $this->request->redirect('filter/auto/city_'.$this->_selected_city_ids['has_cars'], 301);
    }
...
//	    if (!$service->loaded() || !$service->active)
        ...
        $visitor_data = json_encode(array(
            'data' => Encrypt::instance('statistics')->encode(serialize(array(
                'uri' => $this->request->uri(),
                'time_created' => strtotime(Date::formatted_time()),
                'client_ip' => Request::$client_ip,
                'referrer'  => (isset($_SERVER['HTTP_REFERER'])) ? $this->request->referrer() : 'havent_referrer'
            )))
        ));
        ...
        $this->view = View::factory('frontend/services/view_service')
                          ...
                          ->set('service', $service)
                          ->set('images', $service->images->find_all())
                          ->set('news', $service->news->get_news())
                          ->set('stocks', $service->stocks->get_stocks())
                          ->set('vacancies', $service->vacancies->get_vacancies())
                          ->set('reviews', $service->reviews->get_reviews())
                          ->set('cars', $cars)
    ...
    public function action_search_by_car()
    {
        ...
        $content = $current_car['current_object']->contents->where('city_id', '=', $city_id)->find();

        $i18n_params = array(
            ':car_full_name' => $current_car['full_name'],
            ':car_name_ru' => $current_car['name_ru'],
            ':car_name' => $current_car['name'],
            ':normal_city_name' => $this->_geo_params['cities']['total'][$this->_selected_city_ids['has_cars']]->name,
            ':genitive_city_name' => $this->_geo_params['cities']['total'][$this->_selected_city_ids['has_cars']]->genitive_name,
        ...
        if (!trim(strip_tags($content->text)))
            $content->text = __(Kohana::$config->load('default_search_content.search_by_car'), $i18n_params);
    ...
    public function action_search_by_work()
    {
        ...
        $content = $current_work['current_object']->contents->where('city_id', '=', $city_id)->find();
        $i18n_params = array(
            ':work_name' => $current_work['name'],
            ':normal_city_name' => $this->_geo_params['cities']['total'][$this->_selected_city_ids['has_works']]->name,
            ':genitive_city_name' => $this->_geo_params['cities']['total'][$this->_selected_city_ids['has_works']]->genitive_name,
            ':dativus_city_name' => $this->_geo_params['cities']['total'][$this->_selected_city_ids['has_works']]->dativus_name,
        );
        ...
        /* Default text */
        if (!trim(strip_tags($content->text)))
            $content->text = __(Kohana::$config->load('default_search_content.search_by_work'), $i18n_params);
    ...
    function action_search_by_metro()
    {
        ...
        $i18n_params = array(
            ':metro_name' => $this->_geo_params['metro_stations'][$params['metro']['value']]['obj']->name
        );
        if (!trim(strip_tags($content->text)))
            $content->text = __(Kohana::$config->load('default_search_content.search_by_metro'), $i18n_params);
    ...
    public function action_search_by_district()
    {
        ...
        $i18n_params = array(
            ':district_name' => $current_district->name
        );
        if (!trim(strip_tags($content->text)))
            $content->text = __('search_district_default_content', $i18n_params);
