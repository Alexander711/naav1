...
Class HTML extends Kohana_HTML
{
     ...
     * @return string
     */
    static function activate_checker($id, $status = 0, $url)
    {
        if ($status == 1)
        {
            $result = 'Активен '.HTML::anchor($url.'/deactivate/'.$id, '<i class="icon-ban-circle icon-white"></i> Деактивировать', array('class' => 'btn btn-small btn-danger', 'title' => 'Деактивировать'));
        }
        else
        {
            $result = 'Неактивен '.HTML::anchor($url.'/activate/'.$id, '<i class="icon-ok-sign icon-white"></i> Активировать', array('class' => 'btn btn-small btn-success', 'title' => 'Активировать'));
        }
        return $result;
    }
    /**
     ...
     * @return string
     */
    static function delete_button($id, $url)
    {
        return HTML::anchor($url.'/delete/'.$id, '<i class="icon-remove-sign"></i>', array ('class' => 'btn btn-small', 'title' => 'Удалить'));
    }
    /**
     ...
     * @param $id
     * @param $url
     * @param bool $optional_action
     * @return string
     */
    static function edit_button($id, $url, $optional_action = FALSE)
    {
        $anchor_url = (!$optional_action) ? $url.'/edit/'.$id : $url.'/'.$id;
        //return HTML::anchor($anchor_url, HTML::image('assets/img/admin/pencil.png'), array('title' => 'Редактировать', 'class' => 'btn small'));
        return HTML::anchor($anchor_url, '<i class="icon-pencil"></i>', array('title' => 'Редактировать', 'class' => 'btn btn-small'));
    }
    /**
     ...
     * @return string
     */
    static function add_notice($id, $type = 'service')
    {
        if ($type != 'service' AND $type != 'user')
        {
            return;
        }
        return HTML::anchor('admin/notice/add_'.$type.'/'.$id, '<i class="icon-envelope icon-white"></i>', array('class' => 'btn btn-primary btn-small', 'title' => 'Отправить уведомление'));
    }
    /**
     * Кнопка просмотра
     * @static
    ...
    {
        return HTML::anchor($url, '<i class="icon-zoom-in"></i>', array('class' => 'btn btn-small', 'title' => 'Просмотреть', 'target' => '_blank'));
    }
    ...
    )
    {
        ...
        while ($i < $max)
        {
            ...
            if ($is_html)
            {
                ...
                $s2 = preg_replace_callback($re_tags, array('HTML', 'strip_tags_smart'), $s2);
