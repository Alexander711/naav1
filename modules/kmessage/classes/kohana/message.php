...
class Kohana_Message {
    ...
    public static function show_errors($errors = array())
    {
        $view = View::factory('message/error');
        return $view->set('messages', $errors)->render();
    }
    public static function show_once_error($errors, $name)
    {
        $view = View::factory('message/once_error');
        if (Arr::get($errors, $name))
        {
            return $view->set('error', Arr::get($errors, $name))->render();
        }
        else
        {
            return NULL;
    ...
    public static function show_error($errors, $name)
    {
        return (Arr::path($errors, $name)) ? View::factory('message/rms_error')->set('error', Arr::path($errors, $name))->render() : NULL;
    }
